...
fun_ellipse = @(x, P_data) y_fit(x, P_data, lam, mu, rho, 0);
fun_polygonal = @(x, P_data) y_fit(x, P_data, lam, mu, rho, 0);
...
for group = 1:6
    ...
    plot(P_data, vp_ellipse_data(:, indices(group)), 'ko', 'MarkerFaceColor','w', ...
         'DisplayName',sprintf('Measured v_p (Group %d)', group));
    hold on;
    plot(P_data, vp_uniaxial(P_data, ABC_ellipse(1, group)*1e9, ABC_ellipse(2, group)*1e9, ABC_ellipse(3, group)*1e9, lam, mu, rho, 0), 'r-', 'LineWidth',1.6,...
         'DisplayName',sprintf('Fitted v_p (Group %d)', group));
    xlabel('Uniaxial Stress Pa');
    ylabel('v_p m/s');
    title(sprintf('A=%.2f  B=%.2f  C=%.2f  (GPa)', ABC_ellipse(1, group), ABC_ellipse(2, group), ABC_ellipse(3, group)));
    grid on; legend; box on;
end
...
for group = 1:6
    ...
    plot(P_data, vp_polygonal_data(:, indices(group)), 'ko', 'MarkerFaceColor','w', ...
         'DisplayName',sprintf('Measured v_p (Group %d)', group));
    hold on;
    plot(P_data, vp_uniaxial(P_data, ABC_polygonal(1, group)*1e9, ABC_polygonal(2, group)*1e9, ABC_polygonal(3, group)*1e9, lam, mu, rho, 0), 'r-', 'LineWidth',1.6,...
         'DisplayName',sprintf('Fitted v_p (Group %d)', group));
    xlabel('Uniaxial Stress Pa');
    ylabel('v_p m/s');
    title(sprintf('A=%.2f  B=%.2f  C=%.2f  (GPa)', ABC_polygonal(1, group), ABC_polygonal(2, group), ABC_polygonal(3, group)));
    grid on; legend; box on;
end
