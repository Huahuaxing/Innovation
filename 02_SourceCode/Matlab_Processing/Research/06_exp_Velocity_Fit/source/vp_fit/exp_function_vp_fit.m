...
%% 拟合主程序
group = 6;
initialParams = [2118.9, 1e-10, 100, 0.02];
lb = [0, 0, 0, 0]; ub = [3000, 10, 1e3, 1];

params_nonellipse = zeros(group, 4);
resnorm_nonellipse = zeros(group, 1);
params_ellipse = zeros(group, 4);
resnorm_ellipse = zeros(group, 1);
...
for groupi = 1:group
    ...
    nonellipse_data = nonellipse_vp(:, groupi);
    nonellipse_fit = model_func(params_nonellipse(groupi,:), P);
    ellipse_data = ellipse_vp(:, groupi);
...
pqA = repmat(2120, 1, group);
pqK = zeros(1, group);
pqB = repmat(384, 1, group);
pqD = repmat(0.08, 1, group);
...
legend('Nonellipse','Ellipse','Standard value');
xticks(1:6);
...
%% 计算R²
nonellipse_r2 = zeros(group,1);
ellipse_r2 = zeros(group,1);
for i = 1:group
    y = nonellipse_vp(:, i);
...
%% 保存结果到txt文件
reportName = "vp_fit_results_" + string(datetime('now'),'yyyyMMdd_HHmmss') + ".txt";
...
fprintf(fid,'Nonellipse与Ellipse数据对比分析\n--------------------------------------------------\n');
for i = 1:group
    fprintf(fid,'组 %d (%s):\n', i, subTitleList{i});
    fprintf(fid,'  Nonellipse R² = %g\n', nonellipse_r2(i));
    fprintf(fid,'  Ellipse R² = %g\n', ellipse_r2(i));
    fprintf(fid,'  R²差异 = %g\n', abs(nonellipse_r2(i)-ellipse_r2(i)));
    if nonellipse_r2(i) > ellipse_r2(i)
        fprintf(fid,'  Nonellipse拟合效果更好\n\n');
    elseif ellipse_r2(i) > nonellipse_r2(i)
        fprintf(fid,'  Ellipse拟合效果更好\n\n');
    else
        fprintf(fid,'  两种数据拟合效果相当\n\n');
    end
end
fprintf(fid,'统计摘要\n--------------------------------------------------\n');
fprintf(fid,'Nonellipse数据平均R²: %g\n', mean(nonellipse_r2));
fprintf(fid,'Ellipse数据平均R²: %g\n', mean(ellipse_r2));
fprintf(fid,'Nonellipse数据R²标准差: %g\n', std(nonellipse_r2));
fprintf(fid,'Ellipse数据R²标准差: %g\n', std(ellipse_r2));
fprintf(fid,'Nonellipse数据平均残差平方和: %g\n', mean(resnorm_nonellipse));
fprintf(fid,'Ellipse数据平均残差平方和: %g\n', mean(resnorm_ellipse));
fprintf(fid,'\n%s\n报告结束\n', repmat('=',1,80));
fclose(fid);
