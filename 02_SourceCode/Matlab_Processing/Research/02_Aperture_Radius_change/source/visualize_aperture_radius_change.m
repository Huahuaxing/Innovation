...
% 图像保存路径设置
saveBaseDir = 'D:\Projects\02_Innovation\02_SourceCode\Matlab_Processing\Research\02_Aperture_Radius_change\results\01_threshold_algorithm';
...
% P-Radius 多模型对比图
figure('Color',[1 1 1], 'Position', [0 0 2000 1000]);
sgtitle('Uniaxial Stress-Radius', 'FontSize', 14, 'FontWeight','bold');
ax = gobjects(1, 6);
for i = 1:6
    ax(i) = subplot(2, 3, i);
    plot(P, radiusEllipse(:, i), '-b', 'LineWidth', 1.5); hold on;
    plot(P, radiusEllipseAligned(:, i), '-g', 'LineWidth', 1.5);
    plot(P, radiusNonEllipse(:, i), '-r', 'LineWidth', 1.5);
    title(subTitleList{i}, 'FontSize', 12, 'FontWeight','normal');
    xlabel('Uniaxial Stress (MPa)', 'FontSize', 11);
    ylabel('Radius (m)', 'FontSize', 11);
    if i == 1
        legend({'Ellipse model', 'EllipseAligned model', 'Nonelliptical model'}, 'FontSize', 9);
    end
    text(0.5, 0.98, modelParams{i}, ...
...
saveas(gcf, fullfile(saveDirContain, 'Uniaxial Stress-Radius.fig'));
saveas(gcf, fullfile(saveDirContain, 'Uniaxial Stress-Radius.png'));
