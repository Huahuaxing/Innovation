...
for sub = 1:5  % 五个子模型
    ...
    for tab = 1:size(aa, 1)  % 四十个表格
        file_path = fullfile(dataPathEllipseAligned, sprintf('20-cracks-distance-1-%d-AR1', sub), sprintf('20-cracks-distance-%d~40-%d-AR1.txt', tab, 1));
...
for sub = 1:5  % 五个子模型
    ...
    for tab = 1:size(aa, 1)  % 四十个表格
        file_path = fullfile(dataPathEllipseAligned, ...
    ...
    for sub = 1:5
        ...
        for tab = 1:size(aa, 1)
            file_path = sprintf('%s/20-cracks-distance-%d-%d-%s/20-cracks-distance-%d~40-%d-%s.txt', ...
                dataPathEllipseAligned, groupLabel, sub, ARList{arIdx}, tab, sub, ARList{arIdx});
...
save('D:\Projects\02_Innovation\06_ProcessedData\01_aperture_radius_record\EllipseAligned_Record.mat', 'aperture_record', 'radius_record');
