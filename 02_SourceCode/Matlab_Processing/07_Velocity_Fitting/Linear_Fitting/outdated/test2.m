...
dvdpEllipseAligned = zeros(length(P(1:3:end)), 6);
for g=1:groupNum
    dvdpEllipseAligned(:, g) = gradient(vpEllipseAligned(1:3:end,g), P(1:3:end));
end
[~, idx_max] = max(dvdpEllipseAligned);
idx_max = 1 + (idx_max - 1) * 3;
win = 25;
...
for g = 1:groupNum
    ...
    ellipseAligned_fit = slopeEllipseAligned(g) * PInterval{g} + interceptEllipseAligned(g);
    ...
    plot(PInterval{g}/1e6, ellipseAligned_fit, 'g-', 'LineWidth', 1.2, 'DisplayName','Fitting line for ellipseAligned model');
