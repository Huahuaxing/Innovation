...
for g = 1:groupNum
    ...
    % 非椭圆：三角形 + 黑线
    p2 = plot(P/1e6, vpNonellipse(:,g), '-^k', ...
        'LineWidth', LINE_WIDTH, ...
        'MarkerSize', MARKER_SIZE, ...
        'MarkerFaceColor', 'w', ...
        'MarkerIndices', markerIndex);
    ...
    % ===================== 拟合公式文字 =====================
    ellipseStr = sprintf('Elliptical: V_P = %.2eP + %.1f', ...
        slopeEllipse(g), interceptEllipse(g));

    nonellipseStr = sprintf('Nonelliptical: V_P = %.2eP + %.1f', ...
        slopeNonellipse(g), interceptNonellipse(g));

    text(0.95,0.18,ellipseStr, ...
        'Units','normalized', ...
        'FontSize',10, ...
        'FontName',FONT_NAME, ...
        'HorizontalAlignment','right', ...
