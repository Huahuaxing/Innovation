...
# ------------------------- 应力数组 -------------------------
P = np.linspace(3, 600, 200) * 1e5 # 压力范围 3-600 bar，转换为 Pa
...
E = 7.820e9         # 杨氏模量 (Pa)
nu = 0.23           # 泊松比 (无量纲)
lam = 2.71e9        # 拉梅第一参数 (Pa)
# lam = E * nu / ((1 + nu) * (1 - 2 * nu))  # 拉梅常数计算公式
...
# -------------------- 波速计算 2.对称面内如sita2=0--------------------
def vp_uniaxial(P, A, B, C, lam, mu, rho, theta1_deg=0):
    a11 = A11(P, A, B, C, lam, mu)
    a55 = A55(P, A, B, lam, mu)
    K = K_expr(P, A, B, C, lam, mu, theta1_deg)
    return np.sqrt((a11 + a55 + np.sqrt(K)) / rho)

def vsv_uniaxial(P, A, B, C, lam, mu, rho, theta1_deg=0):
    a11 = A11(P, A, B, C, lam, mu)
    a55 = A55(P, A, B, lam, mu)
    K = K_expr(P, A, B, C, lam, mu, theta1_deg)
    return np.sqrt((a11 + a55 - np.sqrt(K)) / rho)

def vsh_uniaxial(P, A, B, lam, mu, rho):
    a44 = A44(P, A, B, lam, mu)
    return np.sqrt(a44 / rho)
...
#         # 在拟合前归一化数据
#         P_scaled = P / 1e6  # 压力单位转换为MPa
#         a44_scaled = a44_data / 1e9  # 弹性常数单位转换为GPa
...
#         params = model.make_params(A=10, B=10)  # 初始猜测值 (GPa)
