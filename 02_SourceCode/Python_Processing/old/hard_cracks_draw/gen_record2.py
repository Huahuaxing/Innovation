...
radius_record = np.zeros((5, 20, 200, 3))
aperture_record = np.zeros((5, 20, 200, 3))
...
for xd in range(1, 6):  # 五个模型
    for xs in range(1, 41):  # 四十个表格
        file_path = f'{dataPath}\\20-cracks-distance-6-{xd}-16AR4+4AR2\\20-cracks-distance-{xs}~40-{xd}-16AR4+4AR2.txt'
        aa[xs-1, :, :] = np.loadtxt(file_path, encoding='utf-8', skiprows=5)  # 1D：裂隙上下界面的点；2D：不同压力；3D：坐标值
